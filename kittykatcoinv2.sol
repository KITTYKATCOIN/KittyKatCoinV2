...
/**
 *Submitted for verification at Polygonscan.com on 2024-01-26
*/
...
contract CustomToken is ERC20, Ownable {
    ...
    event BuybackPercentageUpdated(uint256 newBuybackPercentage);
    event UtilityPercentageUpdated(uint256 newUtilityPercentage);
    event ListingPercentageUpdated(uint256 newListingPercentage);
    event MarketingPercentageUpdated(uint256 newMarketingPercentage);
    event BuybackWalletUpdated(address newBuybackWallet);
    event UtilityWalletUpdated(address newUtilityWallet);
    ...
    function setMaxTxAmount(uint256 _maxTxAmount) external onlyOwner {
        require(_maxTxAmount > 0, "MaxTxAmount must be greater than 0");
        maxTxAmount = _maxTxAmount * 10**decimals();
        emit MaxTxAmountUpdated(maxTxAmount);
    }

    function setBuybackPercentage(uint256 _buybackPercentage) external onlyOwner {
        require(_buybackPercentage <= 100, "Percentage cannot exceed 100");
        buybackFee = _buybackPercentage;
        emit BuybackPercentageUpdated(buybackFee);
    }

    function setUtilityPercentage(uint256 _newPercent) external onlyOwner {
        require(_newPercent <= 100, "Percentage cannot exceed 100");
        utilityFee = _newPercent;
        emit UtilityPercentageUpdated(utilityFee);
    }

    function setListingPercentage(uint256 _newPercent) external onlyOwner {
        require(_newPercent <= 100, "Percentage cannot exceed 100");
        listingFee = _newPercent;
        emit ListingPercentageUpdated(listingFee);
    }

    function setMarketingPercentage(uint256 _newPercent) external onlyOwner {
        require(_newPercent <= 100, "Percentage cannot exceed 100");
        marketingFee = _newPercent;
        emit MarketingPercentageUpdated(marketingFee);
    }

    function setBuybackWallet(address _newWallet) external onlyOwner {
        buybackWallet = _newWallet;
        emit BuybackWalletUpdated(buybackWallet);
    }

    function setUtilityWallet(address _newWallet) external onlyOwner {
        utilityWallet = _newWallet;
        emit UtilityWalletUpdated(utilityWallet);
    }
    ...
    function setListingWallet(address _newWallet) external onlyOwner {
        listingWallet = _newWallet;
        emit ListingWalletUpdated(listingWallet);
    }
